...
exports.createSection4 = async (req, res) => {
  try {
    ...
    const data = req.body;
    const trueList = [];
    data.trueList.forEach((item) => {
      trueList.push({ label: item });
    });
    const formatData = {
      name: "Phần 4",
      code: "P04",
    ...
    if (deThiDaTonTai === null) {
      const deThiMoi = new DeThi(formatData);
      await deThiMoi.save();
      req.flash("message", "Tạo thành công");
      ...
      const deThi = await DeThi.findOne({ code: "P04" });
      deThi.questions.push(formatData.questions);
      await deThi.save();
      // thông báo tạo thành công
      req.flash("message", "Thêm thành công");
      // res.json(deThi)
...
exports.editSection1234 = async (req, res) => {
  ...
  switch (data.codeDeThi) {
    ...
    case "P04":
      ...
      break;
    case "P020":
      res.render("admin/section-2-hinh-nen-edit.pug", {
        cauHoi: cauHoi,
        deThi: deThi,
      });
    case "P030":
      res.render("admin/section-3-hinh-nen-edit.pug", {
        cauHoi: cauHoi,
        deThi: deThi,
      });
      break;
